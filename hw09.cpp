...
int main()
{
	...
	do {
		...
		cout << endl << "\ta: add a new pet to the list\n" <<endl;		
		cout << endl << "\tc: add a new checkup for a pet\n" << endl;		
		cout << endl << "\tr: remove a pet from the list\n" << endl;		
		cout << endl << "\tp: print all pets on the list\n" << endl;		
		cout << endl << "\tq: quit\n" << endl;
		cin >> ch;
		ch = getchar();
		// End Q6
...
{
	Container *petList = list;
	Pet *addPet;
	
	petList->pet->getBreed = breed;
	petList->pet->getName = name;

	if (petList->next != NULL)
	{
...
Pet* search_pet(string name, string breed)
{
	...
	while (temp != NULL)
	{
		if(temp->pet->getName == name)
		{
			if (temp->pet->getBreed == breed)
			{
				return temp->pet;
			}
		}
...
void remove_pet(string name, string breed)
{
	...
	while (temp->pet->getName == name)
	{
		while (temp->pet->getBreed == breed)
		{
			return;
		}
	}
